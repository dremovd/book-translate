#!/usr/bin/env python3
"""Cron driver for m.qidian.com — runs discover + snapshot in sequence.

Cron entry (run every 12 hours):
    0 */12 * * *  cd /path/to/repo && uv run python3 scripts/qidian-cron.py >> data/qidian/cron.log 2>&1

Exit codes mirror jjwxc-cron / fanqie-cron:
    0   clean
    1   per-novel failures unrecovered
    2   phase crashed
   75   lock contention (cron retries)
"""
...
DISCOVER = REPO / 'scripts' / 'qidian-discover.py'
SNAPSHOT = REPO / 'scripts' / 'qidian-snapshot.py'


def parse_args(argv=None):
    p = argparse.ArgumentParser(description='Run qidian discover + snapshot in sequence.')
    p.add_argument('--sleep', type=float, default=0.5,
                   help='--sleep passed to both phases (default: 0.5).')
    p.add_argument('--limit', type=int, default=0,
                   help='Snapshot --limit (default: 0 = no cap).')
    p.add_argument('--verbose', action='store_true',
                   help='Forward verbose output (default: --quiet).')
...
def main(argv=None):
    ...
    print(f'[{_now()}] qidian-cron start', flush=True)
    ...
    if rc_discover == 75:
        print(f'[{_now()}] qidian-cron skipped (lock contention)', flush=True)
        sys.exit(75)

    rc_snapshot, summary_s = _run_phase('snapshot', snapshot_cmd)
    print(f'[{_now()}] {summary_s}', flush=True)
    print(f'[{_now()}] qidian-cron end', flush=True)
